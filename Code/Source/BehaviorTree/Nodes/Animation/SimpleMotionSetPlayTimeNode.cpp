...
{
    SimpleMotionSetPlayTimeNode::SimpleMotionSetPlayTimeNode(const std::string& name, const Core::BehaviorTreeNodeConfiguration& config)
        : Node(name, config)
    {
    }

    void SimpleMotionSetPlayTimeNode::Reflect(AZ::ReflectContext* rc)
    {
        AZ_UNUSED(rc);
    }

    void SimpleMotionSetPlayTimeNode::RegisterNode(const AZStd::shared_ptr<Core::Registry>& registry)
    {
        registry->DelayNodeRegistration<SimpleMotionSetPlayTimeNode>(NODE_NAME);
    }

    Core::BehaviorTreePortsList SimpleMotionSetPlayTimeNode::providedPorts()
    {
    ...
    Core::BehaviorTreeNodeStatus SimpleMotionSetPlayTimeNode::Tick()
    {
