    ...
    const int* inTris)
{
    ...
    rcChunkedGeometryNode& node = nodes[curNode++];
...
static bool CheckOverlapRect(const float amin[2], const float amax[2], const float bmin[2], const float bmax[2])
{
    bool overlap = true;
...
bool rcCreateChunkedGeometry(const float* verts, const int* tris, int ntris, int trisPerChunk, rcChunkedGeometry* cm)
{
    ...
    cm->nodes = new rcChunkedGeometryNode[nchunks * 4];
    if (!cm->nodes)
        return false;
    ...
    {
        const rcChunkedGeometryNode& node = cm->nodes[i];
        if (const bool isLeaf = node.i >= 0; !isLeaf)
            continue;
