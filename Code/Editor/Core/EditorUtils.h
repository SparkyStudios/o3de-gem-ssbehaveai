...
namespace BT
{
    ...
    inline void CreateFlatbuffersBehaviorTree(flatbuffers::FlatBufferBuilder& builder, const BT::Tree& tree)
    {
        ...
        applyRecursiveVisitor(
            ...
            [&](BT::TreeNode* node)
            {
                ...
                auto tn = Serialization::CreateTreeNode(
                    builder, node->UID(), builder.CreateVector(children_uid), convertToFlatbuffers(node->status()),
                    builder.CreateString(node->name().c_str()), builder.CreateString(node->registrationName().c_str()),
        ...
        for (const auto& node_it : tree.manifests)
        {
            ...
            for (const auto& port_it : manifest.ports)
            {
                ...
                auto port_model = Serialization::CreatePortModel(
                    builder, builder.CreateString(port_name.c_str()), convertToFlatbuffers(port.direction()),
                    builder.CreateString(demangle(port.type()).c_str()), builder.CreateString(port.description().c_str()));
                port_models.push_back(port_model);
            }

            auto node_model = Serialization::CreateNodeModel(
                builder, builder.CreateString(manifest.registration_ID.c_str()), convertToFlatbuffers(manifest.type),
        ...
        auto behavior_tree = Serialization::CreateBehaviorTree(
            builder, tree.rootNode()->UID(), builder.CreateVector(fb_nodes), builder.CreateVector(node_models));
     ...
     */
    inline SerializedTransition SerializeTransition(uint16_t UID, Duration timestamp, NodeStatus prev_status, NodeStatus status)
    {
...
namespace SparkyStudios::AI::Behave::BehaviorTree::Editor::Core
{
    ...
    std::vector<QtNodes::Node*> getChildren(const QtNodes::FlowScene& scene, const QtNodes::Node& parent_node, bool ordered);
    ...
    void NodeReorder(QtNodes::FlowScene& scene, AbstractBehaviorTree& abstract_tree);
